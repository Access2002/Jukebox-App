...
public class JukeboxApp{
	...
	public static void main(String[] args) {		
		...
		if(playerChoice == 2) {
			AudioPlayer player = new WavPlayer();
		}
	 ...
	 * @param jbox
	 */
	private static void readFromFile(Jukebox jbox) {
		
		try {
			String csvFilePath = "C:\\test\\output.csv"; // Change directory
			File csvFile = new File(csvFilePath);
			Scanner fileOutput = new Scanner(csvFile);
			
			while(fileOutput.hasNextLine()) {
				String song = fileOutput.nextLine();
				String[] parts = song.split(",");
				
				String songName = parts[0].trim();
				String songArtist = parts[1].trim();
				int songDuration = Integer.parseInt(parts[2].trim());
				
				String songEncoding = parts[3].trim();
				Genre songGenre = Genre.valueOf(parts[4].trim());
				int songPlays = Integer.parseInt(parts[5].trim());
				
				AudioTrack trk = new AudioTrack(songName, songArtist, songDuration, songGenre, songEncoding);
				trk.setPlays(songPlays);
				jbox.addTrack(trk);
			}
	...
	private static void exportStats(AudioTrack[] tracks, String csvOutputPath) {
		
		
		try {
			PrintWriter pw = new PrintWriter("C:\\test\\output.csv");
			for(int i = 0; i < tracks.length; i++) {
				pw.printf(tracks[i].getTitle() + ", ");
				pw.printf(tracks[i].getArtist() + ", ");
				pw.printf(tracks[i].getDuration() + ", ");
				pw.printf(tracks[i].getEncoding() + ", ");
				pw.printf(tracks[i].getStyle() + ", ");
				pw.printf(tracks[i].getPlays() + ", ");
				pw.printf("\n");
			}
	...
	private static void processChoice(int choice, Jukebox jbox) {
		...
		case 8:
			...
			AudioTrack[] tracks = jbox.getTracks(); 
			exportStats(tracks, "JukeboxSongs.csv");
			break;
		default:
